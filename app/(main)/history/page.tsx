'use client'

import { useState } from 'react'
import { useApp } from '@/lib/store'
import { Badge, EmptyState, PageHeader } from '@/components/ui'
import { HISTORY_EVENT_LABEL, HISTORY_EVENT_COLOR, HistoryEventType } from '@/types'
import { formatDateTime, cn } from '@/lib/utils'

const EVENT_TYPES: { key: HistoryEventType | 'all'; label: string }[] = [
  { key: 'all', label: 'ทั้งหมด' },
  { key: 'customer_added', label: 'เพิ่มลูกค้า' },
  { key: 'customer_assigned', label: 'กำหนดเจ้าของดูแล' },
  { key: 'customer_edited', label: 'แก้ไขข้อมูลลูกค้า' },
  { key: 'customer_grade_changed', label: 'เปลี่ยน Grade' },
  { key: 'call_made', label: 'โทร' },
  { key: 'order_created', label: 'ออเดอร์ใหม่' },
  { key: 'order_copied', label: 'คัดลอกข้อมูล' },
  { key: 'order_shipped', label: 'จัดส่ง' },
  { key: 'order_delivered', label: 'ส่งสำเร็จ' },
  { key: 'order_returned', label: 'ตีกลับ' },
  { key: 'order_cancelled', label: 'ยกเลิก' },
  { key: 'commission_paid', label: 'จ่ายคอม' },
  { key: 'member_added', label: 'เพิ่มสมาชิก' },
  { key: 'member_edited', label: 'แก้ไขสมาชิก' },
  { key: 'permission_changed', label: 'เปลี่ยนสิทธิ์' },
]

export default function HistoryPage() {
  const { state } = useApp()
  const [filter, setFilter] = useState<HistoryEventType | 'all'>('all')
  const [search, setSearch] = useState('')

  const filtered = state.history
    .filter(h => filter === 'all' || h.eventType === filter)
    .filter(h => !search || h.description.includes(search) || h.userName.includes(search))

  return (
    <div>
      <PageHeader
        title="ประวัติการทำงาน (History Log)"
        subtitle={`${filtered.length} รายการ`}
        action={
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="ค้นหา..."
            className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 px-3 py-2 text-sm w-56 focus:outline-none focus:ring-2 focus:ring-emerald-400" />
        }
      />

      <div className="p-6 space-y-5">
        <div className="flex gap-1 overflow-x-auto pb-1">
          {EVENT_TYPES.map(t => (
            <button key={t.key} onClick={() => setFilter(t.key)}
              className={cn('px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap',
                filter === t.key ? 'bg-emerald-500 text-white' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800')}>
              {t.label}
            </button>
          ))}
        </div>

        {filtered.length === 0 ? <EmptyState message="ไม่พบประวัติ" /> : (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 divide-y divide-slate-50 dark:divide-slate-800">
            {filtered.map(h => (
              <div key={h.id} className="flex items-start gap-4 px-5 py-4">
                <Badge label={HISTORY_EVENT_LABEL[h.eventType]} className={`${HISTORY_EVENT_COLOR[h.eventType]} shrink-0 mt-0.5`} />
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-slate-800 dark:text-slate-200">{h.description}</p>
                  <p className="text-xs text-slate-400 mt-0.5">{h.userName} · {formatDateTime(h.createdAt)}</p>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
    </div>
  )
}
